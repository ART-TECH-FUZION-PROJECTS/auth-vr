...
(function () {
  ...
  window.amhFetchData = function () {
    ...
    if (tableBody)
      tableBody.innerHTML =
        '<tr><td colspan="5" class="amh-empty-state">Loading...</td></tr>';
    if (mobileBody)
      mobileBody.innerHTML = '<div class="amh-empty-state">Loading...</div>';
        ...
        if (result.success) {
          ...
          if (amhState.searchQuery.length >= 3) {
            ...
            displayData = displayData.filter(function (item) {
              ...
              return (
                e.indexOf(s) > -1 || p.indexOf(s) > -1 || n.indexOf(s) > -1
              );
            });
          }
  ...
  function amhRenderError(msg) {
    ...
    if (tableBody)
      tableBody.innerHTML =
        '<tr><td colspan="5" class="amh-empty-state amh-error-state">' +
        msg +
        "</td></tr>";
    if (mobileBody)
      mobileBody.innerHTML =
        '<div class="amh-empty-state amh-error-state">' + msg + "</div>";
  }
  ...
  function amhRenderTableRows(data) {
    ...
    if (data.length === 0) {
      tableBody.innerHTML =
        '<tr><td colspan="5" class="amh-empty-state">No applications found.</td></tr>';
      mobileBody.innerHTML =
        '<div class="amh-empty-state">No applications found.</div>';
      return;
    }
    ...
    data.forEach(function (app, index) {
      ...
      var statusLabel =
        amhState.tab.charAt(0).toUpperCase() + amhState.tab.slice(1);
      ...
      row.innerHTML =
        '<td class="amh-table-body-cell"><div class="amh-cell-id-wrap">' +
        serialNo +
        "</div></td>" +
        '<td class="amh-table-body-cell">' +
        '<div class="amh-applicant-info">' +
        '<span class="amh-applicant-email">' +
        rawEmail +
        "</span>" +
        ...
        "</td>" +
        '<td class="amh-table-body-cell">' +
        '<span class="amh-status-badge ' +
        amhStatusClass +
        '">' +
        '<span class="amh-status-dot ' +
        amhDotClass +
        '"></span>' +
        statusLabel +
        "</span>" +
        ...
        '<a href="?page=authme-view-form&id=' +
        app.id +
        '" class="amh-action-btn">' +
      ...
      mCard.innerHTML =
        '<div class="amh-t-data-entry"><div class="data-label">S.No</div><div class="data-entry">' +
        serialNo +
        "</div></div>" +
        '<div class="amh-t-data-entry"><div class="data-label">Email</div><div class="data-entry">' +
        rawEmail +
        "</div></div>" +
        '<div class="amh-t-data-entry"><div class="data-label">Mobile Number</div><div class="data-entry">' +
        rawPhone +
        "</div></div>" +
        '<div class="amh-t-data-entry"><div class="data-label">Date/Time</div><div class="data-entry">' +
        app.date +
        "</div></div>" +
  ...
  function amhUpdatePagination(visibleCount, totalCount, totalPages) {
    ...
    if (numbersBox) {
      numbersBox.innerHTML =
        '<button class="amh-page-btn amh-page-btn-active">' +
        amhState.page +
        "</button>";
    }
  }
  ...
  document.addEventListener("DOMContentLoaded", function () {
      ...
      searchInput.addEventListener("input", function (e) {
        ...
        if (
          ...
          if (displayData.length > 0) {
            paginationText.textContent =
              "Showing filtering results (" + displayData.length + ")";
          } else {
            paginationText.textContent = "No results found";
          }
        }
    ...
    if (exportBtn) {
      exportBtn.addEventListener("click", function () {
        var exportUrl =
          authme_admin.ajax_url +
          "?action=authme_admin_export_csv&type=hosts&_wpnonce=" +
          authme_admin.nonce;
